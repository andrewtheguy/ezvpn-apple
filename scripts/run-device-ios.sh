...
# Build, install, and launch EzvpnApp on a paired physical iOS device.
#
...
Builds EzvpnApp, installs it on the given paired device, and launches it.
...
if [[ "$LAUNCH" == "1" ]]; then
  ...
  launch_log="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/ezvpn-launch.XXXXXX")"
